...
static int responseStringsDataRegistrationState(Parcel &p, void *response, size_t responselen) {
    ...
    if (p_cur[3] != NULL) {
        ...
        if (strncmp(p_cur[3], "18", 2) == 0) {
            ALOGE("DATA_REGISTRATION_STATE: stock radioTechnology=18 (QCOM HSPAP_DC) -> cyngn radioTechnology=30 (CM DCHSPAP)");
#else
        if (strncmp(p_cur[3], "19", 2) == 0) {
            ALOGE("DATA_REGISTRATION_STATE: stock radioTechnology=19 (QCOM HSPAP_DC) -> cyngn radioTechnology=30 (CM DCHSPAP)");
#endif
