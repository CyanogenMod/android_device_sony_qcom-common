...
*
* This file wraps a vendor light module.
*
*/

#define LOG_NDEBUG 0

#define LOG_TAG "LightsWrapper"
...
/*******************************************************************
 * implementation of light_device functions
 *******************************************************************/

int lights_set_light(struct light_device_t* dev,
                     struct light_state_t const* state)
{
...
/*******************************************************************
 * implementation of light_module functions
 *******************************************************************/
...
static int get_max_brightness() {
    ...
    if ((fd = open(MAX_BRIGHTNESS_FILE, O_RDONLY)) < 0) {
        ALOGV("[%s]: Could not open max brightness file %s: %s", __FUNCTION__,
                     MAX_BRIGHTNESS_FILE, strerror(errno));
        ALOGV("[%s]: Assume max brightness 255", __FUNCTION__);
        return 255;
    }

    if ((len = read(fd, value, sizeof(value))) <= 1) {
        ALOGV("[%s]: Could not read max brightness file %s: %s", __FUNCTION__,
                     MAX_BRIGHTNESS_FILE, strerror(errno));
        ALOGV("[%s]: Assume max brightness 255", __FUNCTION__);
        close(fd);
        return 255;
...
/* open device handle to one of the lights
 *
 */
                       ...
                       hw_device_t** device)
{
    ...
    if (name != NULL) {
        ...
        if (0 == strcmp(LIGHT_ID_BACKLIGHT, name)) {
            ALOGV("[%s]: %s", __FUNCTION__, "LIGHT_ID_BACKLIGHT");
            set_light = lights_set_light_backlight;
        } else if (0 == strcmp(LIGHT_ID_KEYBOARD, name)) {
            ALOGV("[%s]: %s", __FUNCTION__, "LIGHT_ID_KEYBOARD");
            set_light = lights_set_light;
        } else if (0 == strcmp(LIGHT_ID_BUTTONS, name)) {
            ALOGV("[%s]: %s", __FUNCTION__, "LIGHT_ID_BUTTONS");
            set_light = lights_set_light;
        } else if (0 == strcmp(LIGHT_ID_BATTERY, name)) {
            ALOGV("[%s]: %s", __FUNCTION__, "LIGHT_ID_BATTERY");
            set_light = lights_set_light;
        } else if (0 == strcmp(LIGHT_ID_NOTIFICATIONS, name)) {
            ALOGV("[%s]: %s", __FUNCTION__, "LIGHT_ID_NOTIFICATIONS");
            set_light = lights_set_light;
        } else if (0 == strcmp(LIGHT_ID_ATTENTION, name)) {
            ALOGV("[%s]: %s", __FUNCTION__, "LIGHT_ID_ATTENTION");
            set_light = lights_set_light;
        } else if (0 == strcmp(LIGHT_ID_BLUETOOTH, name)) {
            ALOGV("[%s]: %s", __FUNCTION__, "LIGHT_ID_BLUETOOTH");
            set_light = lights_set_light;
        } else if (0 == strcmp(LIGHT_ID_WIFI, name)) {
            ALOGV("[%s]: %s", __FUNCTION__, "LIGHT_ID_WIFI");
            set_light = lights_set_light;
        } else
            return -EINVAL;
